...
def main():
	wiki_page = "https://en.wikipedia.org/wiki/27th_Canadian_Ministry"
	options = Options()
	...
	table_locator = "//table[@class = 'wikitable']/following-sibling::ul"
	...
	for row in table.find_elements_by_xpath(".//li"):
		...
		for sub_row in row.find_elements_by_xpath(".//ul//li"):
			date = sub_row.text
			name = sub_row.find_element_by_xpath(".//a").text
			date = date.replace(name, "")
